...
**時鐘顯示**
- 雙擊觸控感測器可顯示目前時間與日期；再單擊可瀏覽通知歷史記錄
- 支援 **12 小時制** 與 **24 小時制** — 可從裝置設定選單切換

**動畫播放**
- 從裝置本機儲存中以隨機模式播放 `.qgif` 動畫
- QBIT 閒置時動畫會自動切換

**裝置設定選單**
長按觸控感測器進入設定選單。單擊可捲動選項，長按可進入或切換。選單若 10 秒無操作會自動關閉。選擇 **[ SAVE ]** 後設定才會生效。

| 設定項目 | 說明 |
|---|---|
| QBIT Sound | 開啟或關閉蜂鳴器（動畫與觸控回饋） |
| GIF Invert | 反轉所有動畫的像素顏色 |
| Flip Mode | 將螢幕旋轉 180°（預設為 180° 朝向） |
| Clock Format | 切換 12 小時制或 24 小時制 |

**戳一下通知**
- 接收來自網路上其他 QBIT 使用者的 poke 訊息
- 發送者名稱與訊息內容會顯示於螢幕上，並附有時間戳記
- 戳一下通知支援多國語言顯示：點陣圖在網頁端渲染後才傳至裝置，因此可顯示任意語言
- 雙擊顯示時鐘；再單擊可瀏覽通知歷史記錄

**離線指示**
- 當 QBIT 失去網路連線時，螢幕會顯示視覺提示
